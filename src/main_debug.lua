...
function updateConfig()
	config =
	{
        ...
        REPORT_URL = "https://hooks.slack.com/services/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",
		PATCHNOTES_URL = "http://invisibleincgame.com/updates",
        CRASH_CHANNEL = "#invinc-crashes-DLC1",
		DBG_FILE = "debug.lua",
        CAMPAIGN_HOURS = 72,
		DEV = true,
		RECORD_MODE = false,
...
----------------------------------------------------------------
-- Update Steam Workshop mods if required
-- Gotta load those language mods before initializing the GUI. wee.

local function updateWorkshopMods()
    local settingsFile = savefiles.getSettings( "settings" )

	if KLEISteamWorkshop and settingsFile.data.schedule_mod_update then
        log:write( "Mod update scheduled." )
		settingsFile.data.schedule_mod_update = nil
	    settingsFile:save()

		mod_manager:updateMods()

		while true do
			KLEISteamWorkshop:runSteamCallbacks()

		    local status = KLEISteamWorkshop:getWorkshopUpdateStatus()
		    if not status then
	            log:write( "Mod update failed." )
		        break
		    else
	            -- log:write( "UPDATING: %s, %d", status.state, math.floor(status.progress * 100) )
		    	if status.state == "idle" then
		            log:write( "Mod update complete." )
			        break
			    end
			end
		end
	else
        log:write( "Mod update not scheduled." )
	end
end

updateWorkshopMods()
mod_manager:mountMods()
