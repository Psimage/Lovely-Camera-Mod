...
-- COMOD: Lovely camera
-----------------------
local comod_lovely_simcamera = include( "sim/units/comod_lovely_simcamera" )
-----------------------
-- COMOD: Lovely camera
...
-- COMOD: Lovely camera
-----------------------
	-- We need to include ourselves to the list of units that unit can see to work with the rest of the game code.
	-- Simquery.couldUnitSee returns true for (sim, unit, unit). So basically every unit can see himself.
	-- But we removed ourself cell from the LOS (losCoods) so the resulting seeUnits list doesn't contain us.
	-- And that will cause problems in the future.
	-- TODO: (low) investigate problem more precisely. Can potentially lead to bugs.
	if unit.ClassType and unit.ClassType == "comod_lovely_simcamera" then
		table.insert(seeUnits, unit)
	end
-----------------------
-- COMOD: Lovely camera
