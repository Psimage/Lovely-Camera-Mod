...
function boardrig:updateBlindSpots()
    ...
    sim:forEachUnit(
        function( cameraUnit )
          if cameraUnit:getTraits().mainframe_camera and cameraUnit.ClassType == "comod_lovely_simcamera" then
            local x,y = cameraUnit:getLocation()
            if simquery.isCellWatched( sim, localPlayer, x, y ) == nil then
              local mx,my = self:cellToWorld( x - 0.5, y - 0.5 )
              local MX,MY = self:cellToWorld( x + 0.5, y + 0.5 )
              cover[#cover+1] = mx
              cover[#cover+1] = my
